...
//QQQ??? chenjian
//extern struct v *viterbi_alloc(int len);
//extern void viterbi_free(struct v *vp);
//extern void viterbi_init(struct v *vp, int starting_state);
//extern void viterbi_decode(struct v *vp, const COMPUTETYPE *symbols, unsigned char *data, int nbits);
            ...
            gr_vector_void_star &output_items)
{
  ...
  //std::cout << " d_length = " << d_length << " noutput_items = " << noutput_items << std::endl;

  // decode the whole frame in one shot!
  viterbi_init(d_vp, 0);
  // NOTE: d_length includes padding
  if(d_tmpbuf == NULL) {
    d_tmpbuf = (unsigned int *)malloc(512*4*1024);
  }
  for(int i = 0; i < d_length*CHAR_BIT; ++i) {
    d_tmpbuf[i] = in[i];
  }
  viterbi_decode(d_vp, d_tmpbuf, out, d_length-CHAR_BIT);
  //viterbi_decode(d_vp, (const unsigned int *)in, out, d_length-CHAR_BIT);
  //viterbi_decode(d_vp, (const unsigned char *)in, out, d_length-CHAR_BIT);
