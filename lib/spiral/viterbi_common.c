...
struct v *viterbi_alloc(int len) {
  ...
  if (posix_memalign((void**)&vp->decisions, 16,(len+(KTEM-1))*sizeof(decision_t))) {
    free(vp);
    return NULL;
  }
  viterbi_init(vp, 0);
...
#if (KTEM-1<8)
#define ADDSHIFT (8-(KTEM-1))
#define SUBSHIFT 0
#elif (KTEM-1>8)
#define ADDSHIFT 0
#define SUBSHIFT ((KTEM-1)-8)
#else
#define ADDSHIFT 0
#define SUBSHIFT 0
#endif
   ...
   */
  d += (KTEM-1); /* Look past tail */
  while (nbits-- != 0) {
    int k = (d[nbits].w[(endstate>>ADDSHIFT)/32] >> ((endstate>>ADDSHIFT)%32)) & 1;
    endstate = (endstate >> 1) | (k << (KTEM-2+ADDSHIFT));
...
void viterbi_decode(struct v *vp, const COMPUTETYPE *symbols, unsigned char *data, int nbits) {
  ...
  /* Decode block */
  vp->update_blk(vp, symbols, nbits+(KTEM-1));
